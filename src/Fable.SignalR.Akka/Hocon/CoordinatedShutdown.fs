...
module CoordinatedShutdown =
    ...
    type PhaseBuilder<'T when 'T :> IField> (name: string) =
        ...
        member _.DependsOn (state: string list, value: string) =
            field "depends-on" value::state
        [<EditorBrowsable(EditorBrowsableState.Never)>]
        member _.DependsOn (state: string list, value: PhaseBuilder<_>) =
            field "depends-on" value.Name::state
